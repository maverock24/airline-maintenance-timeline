...
import { API_ROUTES, LOGGING_CONFIG, SERVER_CONFIG } from '../utils/constants';
...
morgan.token('response-time-ms', (req: RequestWithTracking, _res: Response) => {
  if (!req.startTime) return '0';
  return `${Date.now() - req.startTime}ms`;
});

morgan.token('request-id', (req: RequestWithTracking) => {
...
export const httpLoggingMiddleware = morgan(detailedFormat, {
  ...
  skip: (req: Request, _res: Response) => {
    ...
    return (
      process.env.NODE_ENV === SERVER_CONFIG.ENVIRONMENTS.PRODUCTION &&
...
export const performanceMonitoringMiddleware = (
  ...
  res.on('finish', () => {
    ...
    // Log slow requests
    if (duration > LOGGING_CONFIG.PERFORMANCE.SLOW_REQUEST_THRESHOLD) {
      // Log requests taking more than threshold
