...
const SelectedItemDisplay: React.FC<{ selectedItem: TimelineItem | null; onDeselect: () => void; items: TimelineItem[]; inline?: boolean }> = ({ selectedItem, onDeselect, items, inline = false }) => {
    const nextUpcomingItem = useMemo(() => {
        if (selectedItem) return null;
        const now = moment();
        return items
          .filter(item => item.start_time.isAfter(now))
          .sort((a, b) => a.start_time.valueOf() - b.start_time.valueOf())[0] || null;
    }, [items, selectedItem]);
        ...
        return (
            <div className="selected-item-section">
                <div className="selected-item-content">
                    <span>No item selected. Click an item on the timeline to see details.</span>
                </div>
            </div>
        );
    }

    const duration = moment.duration(displayItem.end_time.diff(displayItem.start_time)).humanize();
    const titleParts = displayItem.title.split(' | ');
    const displayTitle = titleParts.length > 1 ? titleParts[1] : displayItem.title;
